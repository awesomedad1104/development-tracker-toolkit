...
export async function getStaticProps(context) {
  ...
  const projects = records.map((project) => getProject(project));
    ...
    <>
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="border-1 border-black flex items-center justify-around bg-white mt-4">
          <FontAwesomeIcon
            icon={faSearch}
            className="text-xl h-4 ml-4 mr-3 bg-white text-searchblue"
          />
          <input
            type="text"
          ...
          />
          <button
            className="border-l-1 border-black bg-seafoam h-10 w-28 flex items-center justify-around"
        ...
        <p className="mt-4 italic text-sm text-gray-500 leading-5 font-light">
          Try searching for a street, developer, use (like “residential” or
          “hotel”), status (like “under construction” or “proposed”) or whatever
          is on your mind.
        </p>
